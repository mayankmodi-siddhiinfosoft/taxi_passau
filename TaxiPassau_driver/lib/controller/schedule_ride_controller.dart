...
class ScheduleRideController extends GetxController with WidgetsBindingObserver {
    ...
    try {
      ...
      if (response.statusCode == 200 && responseBody["success"] == "success") {
        ...
        for (var ride in model.data!) {
          ...
          final isScheduled = ride.rideType == "schedule_ride" && ride.scheduleDateTime != null;

          // Add scheduled rides first
          if (isScheduled) {
            scheduledRides.add(ride);
          }

          // Status Based Categorization (including scheduled rides)
          if (status == "pending" || status == "new" || status == "confirmed" || status == "on ride") {
            newRideList.add(ride);
          } else if (status == "completed") {
            completedRideList.add(ride);
          } else if (status == "rejected" || status == "cancelled" || status == "driver_rejected" || status == "user_rejected") {
